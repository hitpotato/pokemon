...
public class SongPlayer {
	...
	// modified for pokemon
	private static Thread player = null;
	private static Thread mapPlayer = null;
	private static Thread battlePlayer = null;
	...
	public static void playMap(EndOfSongListener waiter, String audioFileName) {
		javax.swing.SwingUtilities.invokeLater(new Runnable() {
			@SuppressWarnings("deprecation")
			public void run() {
				if (battlePlayer != null) {
					while (battlePlayer.isAlive()) {
						// hopefully good enough
						battlePlayer.stop();
					}
				}
				mapPlayer = new AudioFilePlayer(audioFileName);

				((AudioFilePlayer) mapPlayer).addEndOfSongListener(waiter);
	...
	public static void playBattle(EndOfSongListener waiter, String audioFileName) {
		javax.swing.SwingUtilities.invokeLater(new Runnable() {
			@SuppressWarnings("deprecation")
			public void run() {
				if (mapPlayer != null) {
					// while (mapPlayer.isAlive()) {
					// hopefully good enough
					mapPlayer.stop();
					// }
				}
				battlePlayer = new AudioFilePlayer(audioFileName);

				((AudioFilePlayer) battlePlayer).addEndOfSongListener(waiter);
